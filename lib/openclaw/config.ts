// ── OpenClaw Gateway Configuration for ClawHealth ──────────
// This module defines the healthcare-specific OpenClaw configuration
// that powers the AI agent backbone of ClawHealth.

// ── System Prompts (declared before use) ─────────────────

const COORDINATOR_SYSTEM_PROMPT = `You are the ClawHealth AI Coordinator, the primary routing agent for a healthcare clinic management platform.
...
const TRIAGE_SYSTEM_PROMPT = `You are the ClawHealth Triage Agent, handling after-hours patient concerns and symptom assessment.
...
const SCHEDULING_SYSTEM_PROMPT = `You are the ClawHealth Scheduling Agent, managing appointment booking with insurance awareness.
...
const BILLING_SYSTEM_PROMPT = `You are the ClawHealth Billing Agent, an expert in medical billing, claims processing, and revenue cycle management.
...
const RX_SYSTEM_PROMPT = `You are the ClawHealth Prescription Manager Agent, handling medication management and patient adherence.
...
const PA_SYSTEM_PROMPT = `You are the ClawHealth Prior Authorization Agent, specializing in prior authorization workflows.
...
export const OPENCLAW_CONFIG = {
  ...
  channels: {
    whatsapp: { enabled: true, mentionPatterns: ["@clawhealth"] },
